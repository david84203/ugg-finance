...
export async function syncDayToSheet(dateStr, dayData) {
  ...
  // 金額一律 round，避免浮點誤差讓 Sheet 出現一長串小數
  const values = [[
    Math.round(dayData.entryFee  || 0),
    Math.round(dayData.memberFee || 0),
    Math.round(dayData.rental    || 0),
    Math.round(dayData.sale      || 0),
    Math.round(dayData.escape    || 0),
    Math.round(dayData.food      || 0),
    Math.round(dayData.extra     || 0),
    Math.round(dayData.total     || 0),
  ]];
